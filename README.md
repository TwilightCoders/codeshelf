...
- **Content search** — each project is indexed (README excerpt, its `.claude/CONTEXT.md` if present, and a description/keywords blurb from its package manifest) into a small searchable corpus, so the header filter and the Cmd+K palette find a project by *what it is*, not just its name. Cmd+K shows a snippet of the matching text. README-less projects (Xcode/C++/etc.) still index via `.claude/CONTEXT.md`.
